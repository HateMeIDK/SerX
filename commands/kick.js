...
module.exports = {
    name: "kick",
    required_userperms: ["KICK_MEMBERS"],
    required_botperms: ["KICK_MEMBERS"],
    execute: async(message, args)=>{
        ...
        if(!target.kickable){
            return await message.channel.send(errorEmbed(guild.locale, "not_kickable"));
        }
        const embed1 = embedder({
            embed: {
                title: Bot.textdata[guild.locale].you_are_kicked.replace(/\{\{guild\}\}/giu, message.guild.name),
                description: Bot.textdata[guild.locale].kick_message
        ...
        const embed2 = embedder({
            embed: {
                title: Bot.textdata[guild.locale].member_was_kicked.replace(/\{\{guild\}\}/giu, message.guild.name).replace(/\{\{member\}\}/giu, target.user.tag),
                description: Bot.textdata[guild.locale].kick_message
                .replace(/\{\{moderator\}\}/giu, message.author.tag)
                .replace(/\{\{moderator_id\}\}/giu, message.author.id)
                .replace(/\{\{reason\}\}/giu, reason),
